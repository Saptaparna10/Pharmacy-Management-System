...
/**
 * Servlet implementation class UpdateVendorProduct
 */
@WebServlet("/updateVendorProduct.do")
public class UpdateVendorProduct extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateVendorProduct() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String vprod_id=(String) hs.getAttribute("vprodid");
		String vid = (String)hs.getAttribute("logname");
		System.out.println("Eta holo: "+vprod_id);
		System.out.println("Eta korche: "+vid);
		PrintWriter out=response.getWriter();
		
		try {
			Connection con=DataConnection.getConnection();
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery("select * from vendor_product where vprod_id='"+vprod_id+"'");
			...
			while(rs.next()){
				...
				RequestDispatcher rd=request.getRequestDispatcher("updateVendorProduct.jsp");
